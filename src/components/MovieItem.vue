<template>
  <div>
    <!-- eslint-disable -->
    <div class="item" v-for="(item, index) in movie" :key="index" @click="goToMovieDetail(item.id);">
      ...
      <div class="movie-des">
        ...
        <p class="genres">
          <span>类别:</span><span v-if="item.genres.length === 0">未知</span><span v-else-if="item.genres">{{ item.genres }}</span>
        </p>
        <p class="cast">
          主演:<span v-for="items in item.casts" :key="items.name" style="margin-right: 4px;">{{ items.name }}</span>
        </p>
        <p class="director">
          导演:<span v-if="item.casts[1]">{{ item.casts[1].name }}</span>
        </p>
